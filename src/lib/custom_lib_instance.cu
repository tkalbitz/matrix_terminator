...
static void
set_eps(struct c_instance* inst)
{
    int max_len = 0;
    int cur_len = 0;
...
int
c_create_instance(const int matrix_width, const int icount,
                  const int * const rules, const size_t rules_len)
{
    if (matrix_width < 2 || matrix_width > MATRIX_WIDTH) {
...
int
c_destroy_instance(const int instance)
{
...
int
c_set_params(int instance, float max, float delta, float eps, int match,
...
int
c_set_def_params(float max, float delta, float eps, int match, int cond_left,
                 int cond_right)
{
    return c_set_params(DEF_INST, max, delta, eps, match, cond_left, cond_right);
}
