...
void
copy(int rows, int cols, const int * a, int * b)
{
	memcpy(b, a, rows * cols * sizeof(int));
}

void
show(FILE * f, int mcount, int mwidth, const int * a)
{
...
void
plus(int rows, int cols, const int * a, const int * b, int * c)
{
...
void
times(int rows, int mid, int cols, const int * a, const int * b, int * c)
{
...
void
eval(int * w, int mwidth, int * individuum, int * result)
{
	assert(w[0] >= 0);
...
void
mutate(int mcount, int mwidth, int * individuum)
{
...
void
path_mutate(int * lhs, int * rhs, int mcount, int mwidth, int * individuum)
{
	...
	for (int i = 0; lhs[i] >= 0; i++) {
		...
		int * pos = individuum + c * mwidth * mwidth + l * mwidth
				+ goal;
		int new = *pos + 1;
		if (new < 1)
			new = 1;
